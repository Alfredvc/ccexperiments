...
function createWatcher() {
  ...
  function tryResolve(filePath) {
    ...
    // Post-clear: only accept a NEW file (created since markClear).
    // CC v2.1.132 always rotates the JSONL on /clear. We previously also
    // fell back to "old file grew past minOffset" — but title-gen writes
    // (`last-prompt`, `ai-title`) land on the OLD file post-/clear and
    // would lock sessionFile there, then drop the new file's events at
    // the `filePath !== sessionFile` guard. If a future CC version stops
    // rotating, this branch will time out and force a revisit.
    if (clearSnapshot) {
